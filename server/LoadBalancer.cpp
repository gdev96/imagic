...
LoadBalancer::LoadBalancer() {
    ...
    initializeClientConnector();

    //CREATE SERVER CONNECTORS
    initializeServerConnectors();
}

void LoadBalancer::initializeClientConnector(){
...
void LoadBalancer::initializeServerConnectors(){
    for(int i=0; i<N_SERVER; i++) {
        server_connector[i] = ConnectorServer(&server_address[i]);
